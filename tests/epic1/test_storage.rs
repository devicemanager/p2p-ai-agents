...
use p2p_ai_agents::storage::{BackendConfig, ConsistencyLevel, StorageManager, StoragePolicy};
...
async fn test_storage_operations() {
    ...
    // Read via manager
    let val = manager
        .get("manager_key", ConsistencyLevel::Strong)
        .await
...
async fn test_redis_storage() {
    ...
    use p2p_ai_agents::storage::{RedisConfig, RedisStorage};
    ...
    match RedisStorage::new(config).await {
        Ok(redis) => {
            ...
            // Read
            let val = redis
                .get("redis_test_key", ConsistencyLevel::Strong)
                .await

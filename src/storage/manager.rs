...
impl Storage for StorageManager {
    async fn get(&self, key: &str) -> Result<Option<Vec<u8>>, StorageError> {
        ...
        self.execute_with_retry(backends, move |backend| {
            let key = key.clone();
            Box::pin(async move { backend.get(&key).await })
        })
        .await
        .map_err(|e| StorageError::Other(e.to_string()))
    }
    ...
    async fn put(&self, key: &str, value: Vec<u8>) -> Result<(), StorageError> {
        ...
        match &self.policy {
            StoragePolicy::Redundant(_) => {
                ...
                for backend_name in &backends {
                    if let Some(backend) = storage_backends.get(backend_name) {
                        let result = backend.put(key, value.clone()).await;
                        let mut metrics = self.metrics.write().await;
            ...
            _ => {
                ...
                self.execute_with_retry(backends, move |backend| {
                    ...
                    Box::pin(async move { backend.put(&key, value_clone).await })
                })
                .await
                .map_err(|e| StorageError::Other(e.to_string()))
            }
...
mod tests {
    ...
    async fn test_storage_operations() {
        ...
        // Test delete
        assert!(manager.delete("test_key").await.is_ok());
        let result = manager.get("test_key").await.unwrap();
    ...
    async fn test_policy_always_use() {
        ...
        assert!(manager.put("key1", b"value1".to_vec()).await.is_ok());
    ...
    async fn test_policy_first_available() {
        ...
        assert!(manager.put("key2", b"value2".to_vec()).await.is_ok());
        let result = manager.get("key2").await.unwrap();
    ...
    async fn test_metrics() {
        ...
        let _ = manager.put("key1", b"value1".to_vec()).await;
